...
usage() {
  cat <<'USAGE'
Usage: scripts/codex-plan-review-loop.sh [options]

Find the first docs/plan/*.md document with status "Ready for code review" that
does not already have a review file, then run Codex to review it and write
...
Options:
  ...
  --dry-run                                       Print the selected plan/review paths and prompt without invoking Codex
  --dangerously-bypass-approvals-and-sandbox      Pass --dangerously-bypass-approvals-and-sandbox to codex exec (default)
  --no-dangerously-bypass-approvals-and-sandbox   Do not pass --dangerously-bypass-approvals-and-sandbox
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-dangerously-bypass-approvals-and-sandbox)
      bypass_approvals_and_sandbox=0
      shift
      ;;
    --codex-bin)
      codex_bin="$2"
...
run_codex_for_plan() {
  ...
  if [[ "$bypass_approvals_and_sandbox" -eq 1 ]]; then
...
while true; do
  if reviewable_plan="$(find_reviewable_plan)"; then
    if ! run_codex_for_plan "$reviewable_plan"; then
      echo "Codex exited non-zero for $reviewable_plan" >&2
    fi
  else
