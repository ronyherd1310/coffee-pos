...
Current repository status: these commands do not exist yet because the app has not been scaffolded.
...
- Seeder creates the Coffee category.
- Seeder creates Americano at Rp18.000 and Latte at Rp25.000.
- Seeder creates required Temperature options: Hot and Iced.
- Seeder creates required Sugar options: Normal, Less sugar, and No sugar.
...
- MVP menu items:
  ...
  - Latte, Rp25.000.
- MVP modifier groups:
  - Temperature: required single select with Hot and Iced.
  - Sugar: required single select with Normal, Less sugar, and No sugar.
...
- Cashier can add Americano and Latte.
- Cashier can adjust item quantity.
- Cashier must choose one Temperature option and one Sugar option per item.
...
- Total updates from item price, quantity, and modifier price deltas.
...
- The cashier can add Americano and Latte items.
- The cashier can set item quantity.
- Each order item requires one Temperature option and one Sugar option.
...
- The `Confirm Paid` action is disabled until the cart has at least one valid order line and a payment method.
...
- Confirming payment shows a confirmation dialog with order total and payment method before persisting the paid order.
...
| MENU                          | CURRENT ORDER                                  |
|                               |                                                |
| [ Americano ] Rp18.000        | 1x Americano                         Rp18.000 |
| [ Latte     ] Rp25.000        |    Hot, Normal sugar                           |
|                               |    [-] [1] [+]                       [Remove] |
| SELECTED ITEM                 |                                                |
| Americano                     | 1x Americano                         Rp18.000 |
|                               |    Iced, No sugar                              |
| Temperature                   |    [-] [1] [+]                       [Remove] |
| (x) Hot  ( ) Iced             |                                                |
|                               |                                                |
| Sugar                         | [Confirm Paid]             [Print disabled]  |
| (x) Normal                    | Status: Not paid                               |
| ( ) Less sugar                |                                                |
| ( ) No sugar                  | Order note                                      |
|                               | [____________________________________________] |
| Quantity                      |                                                |
| [-] [1] [+]                   | Subtotal                             Rp36.000 |
| [Add Item To Order]           | Payment: [ Cash ] [ QRIS ]                    |
+-------------------------------+------------------------------------------------+
```
...
|              [ Static QRIS Image ]             |
|                                                |
| Cashier checks customer payment manually.      |
|                                                |
| [Confirm Paid]                                 |
+------------------------------------------------+
```
...
- Confirmation dialog behavior for `Confirm Paid` and `Cancel Order`.
...
- Cashier confirms payment through the confirmation dialog and lands on Paid Order Detail.
...
- Cashier can create a paid Americano or Latte order with required modifiers after signing in.
...
- Cashier must confirm payment in a dialog before the paid order is persisted.
- Cashier can select Cash or QRIS only.
- QRIS payment flow displays `/qris/static-qris.png` and relies on manual cashier confirmation.
- Each paid order receives a queue number.
